...
-- 10. Hearings (attorney hearing/court schedule)
...
create or replace trigger hearings_updated_at
  before update on hearings for each row execute function update_updated_at();

alter table hearings enable row level security;
create policy "Allow all for service role" on hearings for all using (true) with check (true);
